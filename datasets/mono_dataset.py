...
class MonoDataset(data.Dataset):
    ...
    def __init__(self,
        ...
        self.use_mild_augmentation = use_mild_augmentation
        ...
        try:
            if use_mild_augmentation:
                # Milder augmentation for homomorphic fusion (方案2-Simple)
                # Reduces augmentation strength to maintain better consistency
                # between RGB features and homomorphic features
                self.brightness = (0.9, 1.1)    # Original: (0.8, 1.2)
                self.contrast = (0.9, 1.1)      # Original: (0.8, 1.2)
                self.saturation = (0.95, 1.05)  # Original: (0.8, 1.2)
                self.hue = (-0.05, 0.05)        # Original: (-0.1, 0.1)
                print("[MonoDataset] Using MILD data augmentation for homomorphic fusion")
            else:
                ...
                self.hue = (-0.1, 0.1)
                print("[MonoDataset] Using STANDARD data augmentation")
    ...
    def __getitem__(self, index):
        ...
        for i in self.frame_idxs:
            # Handle inpaint_pseudo_gt_dir only if it exists (some datasets don't use inpainting)
            if hasattr(self, 'inpaint_pseudo_gt_dir') and self.inpaint_pseudo_gt_dir is not None:
            ...
            if i == "s":
                ...
                if inpaint_pseudo_gt_dir is not None:
        ...
        for i in self.frame_idxs:
            ...
            if ("inpaint_color", i, -1) in inputs:

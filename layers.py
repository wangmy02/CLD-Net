...
def get_smooth_loss(disp, img, input_mask=None):
    ...
    if input_mask is not None:
        expanded_mask = input_mask.expand_as(disp)
        grad_disp_x = grad_disp_x[expanded_mask[:, :, :, :-1].to(torch.bool)]
        grad_disp_y = grad_disp_y[expanded_mask[:, :, :-1, :].to(torch.bool)]
...
def flow_warp(image, flow):
    ...
    Args:
        ...
        flow: [B, 2, H, W] - optical flow (u, v)
              flow[:, 0, :, :] is horizontal displacement (u)
              flow[:, 1, :, :] is vertical displacement (v)
    ...
    # Create meshgrid
    # Backward compatibility for PyTorch < 1.10
    try:
    ...
    grid_x_new = grid_x + flow[:, 0, :, :]  # x + u
    grid_y_new = grid_y + flow[:, 1, :, :]  # y + v
    
    # Normalize to [-1, 1] for grid_sample
    grid_x_new = 2.0 * grid_x_new / (W - 1) - 1.0
    grid_y_new = 2.0 * grid_y_new / (H - 1) - 1.0
    
    # Stack and permute to [B, H, W, 2]
